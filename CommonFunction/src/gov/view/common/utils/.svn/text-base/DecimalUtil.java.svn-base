...
public class DecimalUtil {
    ...
    public static String normalizeNumber(String s,
                                         String s1) throws ParseException {
        s1 = s1.trim();
        ...
        stringbuffer =
                decimalformat.format(number, stringbuffer, new FieldPosition(0));
        return stringbuffer.toString();
    }

    public static String formatDecimal(String s) throws Exception {
        DecimalFormat decimalformat = new DecimalFormat();
        decimalformat.applyPattern("#0.00;-#0.00");
    ...
    public static String formatDecimal(String s, String s1) throws Exception {
        DecimalFormat decimalformat = new DecimalFormat();
        decimalformat.applyPattern("#0.00;-#0.00");
        Number number = decimalformat.parse(s);
    ...
    public static String format(long l, int i) {
        String s = "" + l;
        if (s.length() > i)
            throw new NumberFormatException("Original number length is longer than specified length, number=" +
                                            l + ",len=" + i);
        StringBuffer stringbuffer = new StringBuffer();
        int j = i - s.length();
        for (int k = 0; k < j; k++)
    ...
    public static String restoreDecimal(String s, int i,
                                        boolean flag) throws ParseException {
        if (s.length() <= i)
            throw new ParseException("Number string is too short, must be longer than fraction. str=" +
                                     s + ",fraction=" + i, 0);
        if (i < 0)
        ...
        for (int j = 0; j < ac.length; j++)
            if (!Character.isDigit(ac[j]))
                throw new ParseException("String includes non-digit char:" + s,
                                         j);
    ...
    public static String formatDecimal(double number) throws Exception {
        DecimalFormat decimalformat = new DecimalFormat();
        decimalformat.applyPattern("#0.00;-#0.00");
        String s1 = decimalformat.format(number);
        return s1;
    ...
    public static String formatDecimal(int number) throws Exception {
        DecimalFormat decimalformat = new DecimalFormat();
        decimalformat.applyPattern("#0.00;-#0.00");
        String s1 = decimalformat.format(number);
        return s1;
    ...
    public static String formatDecimalNoSeprator(String s) throws Exception {
        DecimalFormat decimalformat = new DecimalFormat();
        decimalformat.applyPattern("#0.00;-#0.00");
        Number number = decimalformat.parse(s);
        decimalformat.applyPattern("#0.00;-#0.00");
